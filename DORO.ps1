...
<#
.SYNOPSIS
    DORO 压缩包处理脚本（全 WinRAR 版）
.DESCRIPTION
    三层管线：
    1. 将伪装成 .mp4 的压缩包还原为 .zip
    2. 第一层：每个入口解压到 output0\<入口名>\（隔离）
    3. 中间层：output0\<入口名>\ 内每个压缩包解压到 output1\<入口名>\<压缩包名>\（隔离）
    4. 最终层 smart extract：output1\<入口名>\ 内每个压缩包先解到隔离临时目录再判断结构
