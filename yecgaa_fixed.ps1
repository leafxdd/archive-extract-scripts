...
.SYNOPSIS
    yecgaa 压缩包处理脚本 - 优化版
.DESCRIPTION
    1. 将伪装成MP4的压缩包还原为zip并解压到output0
    2. 从output0解压到output
    3. 支持 zip/7z/zip分卷/7z分卷
#>
...
Write-Host "yecgaa 压缩包处理脚本" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# ==================== 步骤 1: 处理 MP4 文件 (使用WinRAR) ====================
Write-Host "步骤 1: 查找 .mp4 文件，重命名为 .zip 并解压到 output0 (WinRAR)" -ForegroundColor Yellow
Write-Host "----------------------------------------"

$mp4ZipSet = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
...
if ($mp4Files.Count -eq 0) {
    ...
    foreach ($file in $mp4Files) {
        ...
        [void]$mp4ZipSet.Add($zipPath)
        
        # 重命名
        try {
            Rename-Item -LiteralPath $file.FullName -NewName $zipName -ErrorAction Stop
            Write-Host "[RENAME] $($file.Name) → $zipName" -ForegroundColor Green
        } catch {
            Write-Host "[ERROR] 重命名失败: $($file.Name) - $_" -ForegroundColor Red
            continue
        }
        
        # 解压到 output0 (使用WinRAR)
        Write-Host "[EXTRACT] $zipName → output0" -ForegroundColor Yellow
        $success = Invoke-WinRARExtract -ArchivePath $zipPath -TargetDir $Output0
        
        if ($success) {
            Write-Host "  ✓ 成功: $zipName" -ForegroundColor Green
            if ($DeleteFlag) {
                Remove-Item -LiteralPath $zipPath -Force -ErrorAction SilentlyContinue
                Write-Host "  → 已删除: $zipName" -ForegroundColor DarkGray
            }
        } else {
            Write-Host "  ✗ 失败: $zipName" -ForegroundColor Red
        }
    }
}
...
# ==================== 步骤 1.5: 处理工作目录中已有压缩包 (使用WinRAR) ====================
Write-Host "步骤 1.5: 处理工作目录中已有 .zip / .7z / 分卷压缩包 → output0 (WinRAR)" -ForegroundColor Yellow
Write-Host "----------------------------------------"

$entries = Get-ArchiveEntrypoints -RootDir $WorkDir -ExcludeDirs @($Output, $Output0)
if ($entries) {
    $entries = @($entries | Where-Object { -not $mp4ZipSet.Contains($_.Path) })
}

if (-not $entries -or $entries.Count -eq 0) {
    Write-Host "未发现其他压缩包" -ForegroundColor Gray
} else {
    foreach ($e in $entries) {
        $label = switch ($e.Type) {
            'zip-z' { 'zip分卷(z01)' }
...
Write-Host "步骤 2: 从 output0 解压 .zip / .7z / 分卷压缩包 → output (7z)" -ForegroundColor Yellow
Write-Host "----------------------------------------"
...
if ($DeleteFlag) {
    ...
    if ($deletedCount -eq 0) {
        ...
        Write-Host "共清理 $deletedCount 个空文件夹（包括 output0）" -ForegroundColor Green
    }
} else {
    Write-Host "KeepFiles 模式，跳过清理" -ForegroundColor Gray
}
